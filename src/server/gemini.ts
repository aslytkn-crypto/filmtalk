...
export async function getMovieRecommendationsServer(userPreferences: string): Promise<Movie[]> {
...
export async function summarizeChatServer(messages: { user: string; text: string }[]): Promise<string> {
...
export async function tagChatServer(messages: { user: string; text: string }[]): Promise<string[]> {
  const chatHistory = messages.map(m => `${m.user}: ${m.text}`).join("\n");
  const response = await ai.models.generateContent({
    model: "gemini-1.5-flash",
